...
your network. See also [ESP32 SSH Server](../SSH-to-ULX3S-ESP32/)

![ssh_uart_ESP8266_HUZZAH.png](../images/ssh_uart_ESP8266_HUZZAH.png)
...
When swapping UART0 and UART2, in our case, we _don't_ want the USB traffic to be seen on the 
external UART for our SSH Server. But we _do_ want to be able to see diagnostic messages that 
are normally viewed on the USB port. The ESPLOG default output is configured with the `menuconfig`
